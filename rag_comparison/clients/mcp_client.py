...
class MCPGraphRAGClient(RAGClient):
    ...
    async def query(
        ...
        try:
            ...
            # Extract retrieval_time_ms from response (fast endpoint exposes this)
            retrieval_time_ms = result.get("retrieval_time_ms", -1)
            if retrieval_time_ms < 0 and "performance" in result:
                # Try to get from performance.retrieval_time_ms
                retrieval_time_ms = result["performance"].get("retrieval_time_ms", -1)
