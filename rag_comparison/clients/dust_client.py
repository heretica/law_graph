...
class DustClient(RAGClient):
    ...
    async def query(self, question: str) -> QueryResult:
        ...
        try:
            ...
            async with session.post(url, json=payload) as response:
                ...
                # Poll for agent response and extract retrieval timing
                answer, retrieval_time_ms = await self._poll_for_response(
                    session, conversation_id, start_time
                )
    ...
    async def _poll_for_response(
        ...
        """Poll conversation for agent message completion and extract retrieval timing.
        ...
        Returns:
            Tuple of (answer, retrieval_time_ms). retrieval_time_ms is -1 if not available.
        """
                        ...
                        for msg in item:
                            if msg.get("type") == "agent_message":
                                ...
                                if status == "succeeded":
                                    ...
                                    # Extract retrieval timing from actions
                                    retrieval_time_ms = self._extract_retrieval_time_from_actions(
                                        msg.get("actions", [])
                                    )
                                    return answer, retrieval_time_ms
                                elif status == "failed":
                                    error = msg.get("error", {})
    ...
    def _extract_retrieval_time_from_actions(self, actions: list) -> float:
        """Extract retrieval time from agent message actions.

        Sums up all semantic_search action durations (updatedAt - createdAt).

        Args:
            actions: List of action objects from agent_message.

        Returns:
            Total retrieval time in milliseconds, or -1 if no retrieval actions found.
        """
        if not actions:
            return -1.0
        ...
        for action in actions:
            # Look for semantic_search or similar retrieval actions
            tool_name = action.get("toolName", "")
            function_name = action.get("functionCallName", "")

            if "search" in tool_name.lower() or "search" in function_name.lower():
                created_at = action.get("createdAt")
                updated_at = action.get("updatedAt")

                if created_at and updated_at:
                    # Timestamps are in milliseconds
                    delta_ms = updated_at - created_at
                    total_retrieval_ms += delta_ms
                    retrieval_found = True

        return total_retrieval_ms if retrieval_found else -1.0
